...
/**
 * Single shared voice-recording store (recording lifecycle + transcription +
 * mic permission), used by every page that captures audio. Only one capture
 * page is mounted at a time; each registers its transcript destination on
 * mount via `setTranscriptHandler`.
 */
