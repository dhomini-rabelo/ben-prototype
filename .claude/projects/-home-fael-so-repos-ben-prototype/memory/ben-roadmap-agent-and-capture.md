...
2. **Capture Card — DONE** (task `.claude/current-tasks/capture-card-feature/`, built 2026-06-01, in-memory per user's choice). Backend: new in-memory `Note`/`Reminder`/`Task` entities + repositories; `PersistCapturesUseCase` persists all drafts (precedence reminders→tasks→notes) and `ResolveCaptureUseCase` rebuilds them for history; `/chat` links the primary item to `Message.capture` and both `/chat` + `/messages/list` now return an enriched `CaptureView { kind, itemId, title, meta }`. Web: `MessageCapture` enriched, `use-chat` attaches `reply.capture` to the Ben message, `chat-history` renders the real card title/meta (was using the Ben message text). Cards are display-only — no start/complete `onAction` yet (no backend endpoints).
3. **Persistence — migrate to a real DB — NEXT.** Decided (backend is in-memory only today) but the DB stack was NOT chosen yet (Prisma/SQLite vs Postgres vs Drizzle — the question was interrupted). This is now the next step.
...
**How to apply:** capture card is shipped; the next build is the DB migration — confirm the DB stack first. Follow-up: wire capture-card interactions (start/complete task) once DB + endpoints exist. See [[ben-prototype-overview]] if present.
